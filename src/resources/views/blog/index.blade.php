            ...
            <div class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                <div class="mb-8 flex flex-wrap items-center gap-4">
                    <form action="{{ route('blog.index') }}" method="get" class="flex gap-2 flex-1 min-w-[200px]">
                        ...
                        <button type="submit" class="fi-btn cursor-pointer inline-flex items-center justify-center rounded-lg bg-amber-500 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-amber-600">Ara</button>
                    </form>
                    <div class="flex flex-wrap gap-2">
                        <a href="{{ route('blog.index') }}" class="fi-btn cursor-pointer inline-flex items-center rounded-lg px-3 py-1.5 text-sm font-medium {{ !request('kategori') && !isset($currentCategory) ? 'bg-amber-500 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600' }}">Tümü</a>
                        @foreach($categories as $cat)
                            <a href="{{ route('blog.category', $cat->slug) }}" class="fi-btn cursor-pointer inline-flex items-center rounded-lg px-3 py-1.5 text-sm font-medium {{ (isset($currentCategory) && $currentCategory->id === $cat->id) ? 'bg-amber-500 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600' }}">{{ $cat->name }}</a>
                        @endforeach
                    </div>
                </div>
