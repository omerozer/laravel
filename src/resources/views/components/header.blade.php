<header class="sticky top-0 z-30 border-b border-white/5 bg-black/90 backdrop-blur-xl">
    <div class="mx-auto {{ $siteWidth ?? 'max-w-7xl' }} px-4 sm:px-6 lg:px-8">
        <div class="flex h-16 items-center justify-between gap-4">
            <a href="{{ route('home') }}" class="text-xl font-semibold text-white hover:text-zinc-200 transition-colors">
                {{ config('app.name') }}
            </a>
            <nav class="flex items-center gap-1 sm:gap-2">
                <a href="{{ route('blog.index') }}" class="px-3 py-2 text-sm font-medium text-zinc-400 hover:text-white transition-colors rounded-lg hover:bg-white/5">
                    Blog
                </a>
                <a href="{{ url('/dashboard') }}" class="px-3 py-2 text-sm font-medium text-zinc-400 hover:text-white transition-colors rounded-lg hover:bg-white/5">
                    Admin
                </a>
                <button type="button" data-theme-toggle class="p-2 text-zinc-400 hover:text-white rounded-lg hover:bg-white/5 focus:outline-none focus:ring-2 focus:ring-amber-500/50 transition-colors" aria-label="Tema değiştir">
